...
void reservation_policy<Clock>::release(group_id g) noexcept {
    ...
    // first try to return the slot to the releasing group. this will dispatch
    // any waiters there. failing that, invoke the policy-level dispatch logic,
    // which computes a dispatch priority across groups.
    if (gs.maybe_return_reserved_slot() || dispatch_next()) {
        return;
    }
...
template<class Clock>
bool reservation_policy<Clock>::dispatch_next() noexcept {
