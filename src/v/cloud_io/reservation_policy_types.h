...
struct reservation_group_state {
    ...
    /// Record a fast-path admit (no queue). from_reserved=true when
    /// the slot came from the group's reservation, false when from the
    /// common pool.
    void on_immediate_admit(bool from_reserved) noexcept {
        track_in_flight(from_reserved);
        ++admit_total;
        ++admit_immediate_total;
    }

    /// Record a slot release on this group.
    ...
    /// Hand a slot to one queued waiter. Precondition: queue
    /// non-empty. from_reserved=true when the slot came from the
    /// group's reservation.
    void dispatch_one(bool from_reserved = false) noexcept {
    ...
    /// If the group has a reserved slot in flight, return it to the
    /// group's reservation (to a same-group waiter if one is queued,
    /// otherwise to `reserved_available`) and return true. Returns
    /// false if the in-flight release should be returned to the common
    /// pool instead.
...
private:
    ...
    /// Bump in-flight counters for a newly-admitted op. Shared by the
    /// fast-path and dispatched-admit code paths.
    void track_in_flight(bool from_reserved) noexcept {
        ++in_flight;
        if (from_reserved) {
            ++reserved_in_flight;
        }
