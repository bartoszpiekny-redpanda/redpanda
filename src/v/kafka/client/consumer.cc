...
ss::future<> consumer::seed_positions_from_committed() {
    ...
    for (const auto& t : res.data.topics) {
        for (const auto& part : t.partitions) {
            ...
            // client's value, matching the Confluent REST Proxy where posting X
            // resumes at X+1). If committed+1 is below the log start after
            // retention, the fetch returns offset_out_of_range and the reseed
            // to log_start recovers it.
...
ss::future<fetch_response> consumer::fetch(
  std::chrono::milliseconds timeout, std::optional<int32_t> max_bytes) {
    ...
    // Seeded with an empty response for the degenerate case where the timeout
    // budget is already spent and the loop body never runs.
    ...
    while (ss::lowres_clock::now() < deadline) {
        ...
        std::tie(result, reseeded) = co_await fetch_round(
        ...
        if (detail::has_records(result) || !reseeded) {
            break;
        }
    }
