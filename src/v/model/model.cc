...
// Payload for wire_key_and_value_schema: the canonicalized config string
// wrapped in a versioned envelope for forward/backward compatibility.
...
parse_new_format(std::string_view str) {
    ...
    if (!key_opts && !value_opts && !headers_opts) {
        return std::unexpected(
          ss::sstring("iceberg_mode config must contain at least one section"));
    }
